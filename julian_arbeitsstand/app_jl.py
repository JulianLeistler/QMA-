...
def plot_historical_performance(portfolio_returns, market_returns, risk_free_returns, start_cap, title="Historische Renditeentwicklung"):
    aligned_data = pd.concat([portfolio_returns.squeeze().rename('Portfolio'), market_returns.squeeze().rename('Market'), risk_free_returns.squeeze().rename('RiskFree')], axis=1, sort=False).dropna()
    
    port_growth = start_cap * (1 + aligned_data['Portfolio']).cumprod()
    mkt_growth = start_cap * (1 + aligned_data['Market']).cumprod()
    rf_growth = start_cap * (1 + aligned_data['RiskFree']).cumprod()

    port_growth.loc[aligned_data.index[0] - pd.Timedelta(days=1)] = start_cap
    mkt_growth.loc[aligned_data.index[0] - pd.Timedelta(days=1)] = start_cap
    rf_growth.loc[aligned_data.index[0] - pd.Timedelta(days=1)] = start_cap
    
    port_growth, mkt_growth, rf_growth = port_growth.sort_index(), mkt_growth.sort_index(), rf_growth.sort_index()

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=port_growth.index, y=port_growth, mode='lines', line=dict(color='rgb(52, 152, 219)', width=2), name='MAG7 Portfolio'))
    fig.add_trace(go.Scatter(x=mkt_growth.index, y=mkt_growth, mode='lines', line=dict(color='rgb(231, 76, 60)', width=2), name='Markt (Benchmark)'))
    fig.add_trace(go.Scatter(x=rf_growth.index, y=rf_growth, mode='lines', line=dict(color='rgb(46, 204, 113)', width=2, dash='dot'), name='Risk-Free Rate'))
    
    fig.update_layout(title=title, xaxis_title="Datum", yaxis_title="Portfoliowert ($)", template="plotly_dark", hovermode="x unified", margin=dict(l=20, r=20, t=50, b=20), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
